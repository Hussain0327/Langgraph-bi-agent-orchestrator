...
class RoutingClassifier:
    ...
    def train(self, data_path='models/training_data.json', num_epochs=10, batch_size=16, learning_rate=2e-05, save_path='models/routing_classifier.pkl'):
        ...
        for agent in self.agent_labels:
            ...
            agent_metrics[agent] = {'precision': float(precision), 'recall': float(recall), 'f1': float(f1), 'accuracy': float(accuracy), 'train_examples': sum(train_binary), 'val_examples': sum(val_binary)}
        ...
        self.training_metrics = {'timestamp': datetime.now().isoformat(), 'base_model': self.base_model_name, 'num_epochs': num_epochs, 'batch_size': batch_size, 'learning_rate': learning_rate, 'agent_metrics': agent_metrics, 'avg_f1': float(avg_f1), 'avg_precision': float(avg_precision), 'avg_recall': float(avg_recall), 'exact_match_accuracy': float(exact_match_accuracy), 'training_examples': len(queries), 'validation_examples': len(val_queries)}
        self.save(save_path)
        print(f'\n Training complete!')
        print(f'=' * 70 + '\n')
        return self.training_metrics

    def predict(self, query: str, threshold: float=0.5, adaptive_thresholds: Dict[str, float]=None) -> List[str]:
        if not self.models:
    ...
    def evaluate(self, data_path='models/training_data.json'):
        ...
        for agent in self.agent_labels:
            ...
            agent_metrics[agent] = {'precision': float(precision_score(true_binary, pred_binary, zero_division=0)), 'recall': float(recall_score(true_binary, pred_binary, zero_division=0)), 'f1': float(f1_score(true_binary, pred_binary, zero_division=0)), 'accuracy': float(accuracy_score(true_binary, pred_binary))}
        avg_f1 = np.mean([m['f1'] for m in agent_metrics.values()])
        metrics = {'test_examples': len(test_queries), 'exact_match_accuracy': float(exact_match_accuracy), 'exact_matches': exact_matches, 'agent_metrics': agent_metrics, 'avg_f1': float(avg_f1)}
    ...
    def save(self, path: str='models/routing_classifier.pkl'):
        ...
        try:
            ...
            save_data = {'models': self.models, 'agent_labels': self.agent_labels, 'base_model_name': self.base_model_name, 'training_metrics': self.training_metrics}
...
def main():
    ...
    parser.add_argument('--data', type=str, default='models/training_data.json', help='Path to training data')
    parser.add_argument('--output', type=str, default='models/routing_classifier.pkl', help='Path to save trained model')
    parser.add_argument('--epochs', type=int, default=10, help='Number of training epochs')
    parser.add_argument('--batch-size', type=int, default=16, help='Batch size')
    args = parser.parse_args()
    classifier = RoutingClassifier()
    metrics = classifier.train(data_path=args.data, num_epochs=args.epochs, batch_size=args.batch_size, save_path=args.output)
